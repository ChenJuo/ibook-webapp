    ...
    export default {
        ...
        mounted(){
            ...
            this.task = setInterval(() =>{
                that.$refs.mask.forEach((item,index) =>{
                    let mask = that.$refs.mask[index];
                    let line = that.$refs.line[index];
                    let maskWidth = that.maskWidth[index];
                    let lineWidth = that.lineWidth[index];
                    if(index === 0){
                        if(that.add && maskWidth.value < 16){
                            maskWidth.value++;
                            lineWidth.value--;
                        }else if(!that.add && lineWidth.value < 16){
                            maskWidth.value--;
                            lineWidth.value++;
                        }
                    }else{
                        if(that.add && maskWidth.value < 16){
                            let preMaskWidth = that.maskWidth[index - 1]
                            if(preMaskWidth.value >= 8){
                                maskWidth.value++;
                                lineWidth.value--;
                            }
                        }else if(!that.add && lineWidth.value <16){
                            let preLineWidth = that.lineWidth[index - 1]
                    ...
                    if(index === that.maskWidth.length - 1){
                        if(that.add){
